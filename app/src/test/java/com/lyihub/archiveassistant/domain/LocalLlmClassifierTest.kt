...
class LocalLlmClassifierTest {
    ...
    fun classifySuccess() = runTest {
        val classifier = classifierReturning("""{"topic": "officials", "confidence": 0.9, "reason": "match"}""")
    ...
    fun emptyTopicsListStillUsesSixMinistryIds() = runTest {
        ...
        assertTrue(result is ClassificationResult.Classified)
        assertEquals(SixMinistry.OFFICIALS.id, (result as ClassificationResult.Classified).payload.topicId)
    }

    @Test
    fun veryLongRawInput_doesNotCrashAndPreservesInput() = runTest {
        val rawInput = "Long input ".repeat(1_100)
        val classifier = classifierReturning("""{"topic": "officials", "confidence": 0.9, "reason": "long match"}""")
    ...
    fun confidenceOutOfRange_stillParsesMatchingTopic() = runTest {
        val classifier = classifierReturning("""{"topic": "officials", "confidence": 1.5, "reason": "match"}""")
    ...
    fun inferenceTimeout() = runTest {
        val engine = initializedEngine("""{"topic": "officials", "confidence": 0.9, "reason": "slow"}""").apply {
    ...
    fun unknownTopicFallsBackToTreasury() = runTest {
        ...
        assertTrue(result is ClassificationResult.Classified)
        assertEquals(SixMinistry.TREASURY.id, (result as ClassificationResult.Classified).payload.topicId)
    }

    @Test
    fun legacyTopicOutputFallsBackToTreasury() = runTest {
        val classifier = classifierReturning("""{"topic": "topic-ai-architecture", "confidence": 0.9, "reason": "legacy"}""")
    ...
    fun promptMentionsOnlySixMinistryIds() = runTest {
        val engine = initializedEngine("""{"topic": "officials", "confidence": 0.9, "reason": "match"}""")
        val classifier = LocalLlmClassifier(engine)
